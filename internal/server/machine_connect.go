...
func (s *machineConnectService) StartMachine(ctx context.Context, req *connect.Request[arcav1.StartMachineRequest]) (*connect.Response[arcav1.StartMachineResponse], error) {
	...
	templateID, err := s.resolveStartTemplateID(ctx, machine, req.Msg.GetTemplateId())
	if err != nil {
		return nil, err
	}

	if strings.TrimSpace(templateID) != strings.TrimSpace(machine.TemplateID) {
		// Re-snapshot template config when switching templates
		tmpl, tmplErr := s.store.GetMachineTemplateByID(ctx, templateID)
		if tmplErr != nil {
			log.Printf("get template for snapshot failed: %v", tmplErr)
			return nil, connect.NewError(connect.CodeInternal, errors.New("failed to resolve template"))
		}
		templateUpdated, updateErr := s.store.UpdateMachineTemplateByIDForOwner(ctx, userID, machineID, templateID, currentSetupVersion(), tmpl.Type, tmpl.ConfigJSON)
		if updateErr != nil {
			log.Printf("set machine template id failed: %v", updateErr)
			return nil, connect.NewError(connect.CodeInternal, errors.New("failed to select template"))
		}
		if !templateUpdated {
			return nil, connect.NewError(connect.CodeNotFound, errors.New("machine not found"))
		}
...
func (s *machineConnectService) resolveStartTemplateID(ctx context.Context, machine db.Machine, requestedTemplateID string) (string, error) {
	templateID := strings.TrimSpace(requestedTemplateID)
	if templateID == "" {
		templateID = strings.TrimSpace(machine.TemplateID)
	}
	if templateID == "" {
		return "", connect.NewError(connect.CodeFailedPrecondition, errors.New("machine template is not configured"))
	}
